function shouldBehaveLikeTokenMintable(proxyOwner, tokenOwner, owner, anotherAccount) {
  const expectRevert = require('../helpers/expectRevert')

  describe('initialize', function () {
    it('can not be initialized twice', async function () {
      await expectRevert(this.token.initialize(anotherAccount))
    })
  })
  ...
  describe('transfer ownership', function () {
    describe('when the new proposed owner is not the zero address', function () {
      ...
      describe('when the sender is the proxy owner', function () {
        ...
        it('reverts', async function () {
          await expectRevert(this.token.transferOwnership(newOwner, { from }))
        })
      })
    ...
    describe('when the new proposed owner is the zero address', function () {
      ...
      it('reverts', async function () {
        await expectRevert(this.token.transferOwnership(newOwner, { from: tokenOwner }))
      })
    })
  })
  ...
  describe('finish minting', function () {
    describe('when the sender is the token owner', function () {
      ...
      describe('when the token was already finished', function () {
        ...
        it('reverts', async function () {
          await expectRevert(this.token.finishMinting({ from }))
        })
      })
    })
  ...
  describe('mint', function () {
    ...
    describe('when the sender is the token owner', function () {
      ...
      describe('when the token minting is finished', function () {
        ...
        it('reverts', async function () {
          await expectRevert(this.token.mint(owner, amount, { from }))
        })
      })
    })
...
module.exports = shouldBehaveLikeTokenMintable